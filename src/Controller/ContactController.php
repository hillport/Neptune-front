...
class ContactController extends AbstractController
{
    ...
    /**
     * @Route("api/{_locale}/sponsor",name="api_sponsor",requirements={"_locale"="[a-z]{2}"})
     */
    public function sponsor(Request $request, \Swift_Mailer $mailer){


        // Création du formulaire de contact
        $contactRequest = new ContactRequest();

        $form = $this->createForm(SponsorForm::class , $contactRequest,array(
            'action'    =>  $this->generateUrl('api_contact',array('_locale'=> $request->getLocale())),
        ));
            ...
            if($form->isValid()){
                if(!empty($request->get('g-recaptcha-response'))){
                    if($this->validateRecaptcha('6Le41GwUAAAAAH5xkR6Z6sDsoe17GYDyNQHJ0Pbh',$_POST['g-recaptcha-response']) === true || getenv('APP_ENV') === 'dev'){
                        ...
                        if($this->container->hasParameter('activeMimeTypes') && $this->container->hasParameter('activeExtensions')){
                            die('ok');
                            foreach ($form as $val){
                                if(!$val instanceof FormInterface)
                                    continue;
